...
@testset "Estimate Delay" begin
    ...
    ds = Systems.henon()
    data = trajectory(ds,1000;dt=1, alg = Vern9())
    x = data[:,1]
    ...
    dt = 0.02
    data = trajectory(ds,500,dt=dt,alg = Vern9())
    x = data[:,1]
    ...
    dt = 0.1
    data = trajectory(ds,2000,dt=dt,alg = Vern9())
    x = data[:,1]
    ...
    ds = Systems.lorenz()
    dt = 0.1
    data = trajectory(ds,5000;dt=dt,alg = Vern9())
    x = data[500:end,1]
